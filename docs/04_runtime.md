# runtime layer（最小限 C ランタイム機能）

## 1. はじめに

runtime layer は、OS や標準 libc に依存せずに C プログラムを実行するための
最小限の機能に絞って C ランタイム機能（セクション初期化・基本関数）を提供します。
...
baremetal-lab では、`main()` を実行する前に 初期値付き変数（`.data`） や 初期値なし変数（`.bss`） を初期化し、
`putc()` / `puts()` / `memcpy()` / `memset()` を提供しています。

本章では、これらの構成要素について説明します。
